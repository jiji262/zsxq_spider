...
ZSXQ_ACCESS_TOKEN = 'FD4DBAF2-5D89-C2BA-A896-FB0094A56394_501071308FC2B08E'    # 登录后Cookie中的Token（必须修改）
USER_AGENT = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 11_2_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.36'    # 登录时使用的User-Agent（必须修改）
GROUP_ID = '555188425154'                         # 知识星球中的小组ID
PDF_FILE_NAME = 'Book.pdf'                       # 生成PDF文件的名字
...
def get_data(url):
    ...
    with open(GROUP_ID + '/temp.json', 'w', encoding='utf-8') as f: # 将返回数据写入temp.json方便查看
        f.write(json.dumps(rsp.json(), indent=2, ensure_ascii=False))
    
    with open(GROUP_ID + '/temp.json', encoding='utf-8') as f:
        for topic in json.loads(f.read()).get('resp_data').get('topics'):
            ...
            if DOWLOAD_PICS and content.get('images'):
                ...
                for img in content.get('images'):
                    ...
                    local_url = GROUP_ID + '/images/' + str(num - 1) + '_' + str(images_index) + '.jpg'
                    images_index += 1
...
def make_pdf(htmls):
    ...
    try:
        pdfkit.from_file(html_files, GROUP_ID + '/' + PDF_FILE_NAME, options=options)
